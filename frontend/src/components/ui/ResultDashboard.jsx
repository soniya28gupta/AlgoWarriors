...
import React, { useEffect, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { MapPin, AlertTriangle, CheckCircle, BrainCircuit, Apple, HeartPulse, Moon, Stethoscope, FileText, Download, FlameKindling, Zap, ShieldAlert, XCircle } from 'lucide-react';
import { getHistory } from '../../services/api';
import { generateDoctorReport } from '../../utils/pdfGenerator';
import RecommendationDashboard from './RecommendationDashboard';
import HospitalMap from './HospitalMap';
  ...
  return (
    <div className="relative flex items-center justify-center">
      <svg className="transform -rotate-90 w-40 h-40">
        <circle cx="80" cy="80" r={radius} stroke="#EDE9E6" strokeWidth="10" fill="transparent" />
        <circle
          cx="80"
          cy="80"
          r={radius}
      ...
      <div className="absolute text-center">
        ...
        <div className="text-xs" style={{ color: '#5C4F4A80' }}>{label}</div>
      </div>
    </div>
  );
};
...
export default function ResultDashboard({ result, formData, diseaseType, onReset }) {
  ...
  const iconMap = {
    ...
    Medium: <AlertTriangle className="text-yellow-500" />,
    High: <XCircle className="text-red-500" />,
  };
  ...
  return (
    <div className="min-h-screen p-6" style={{ backgroundColor: '#EDE9E6' }}>
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        >
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div>
              <h2 className="text-2xl font-bold mb-2" style={{ color: '#5C4F4A' }}>
                Diagnostic Result
              </h2>
              ...
              <p className="text-sm" style={{ color: '#5C4F4A80' }}>
                {care_plan?.analysis || "System detected relevant health patterns."}
              </p>
            </div>
        ...
        <div className="p-8 rounded-3xl shadow-xl"
          style={{ backgroundColor: 'rgba(255,255,255,0.95)' }}
        >
          <h3 className="text-xl font-semibold mb-6" style={{ color: '#5C4F4A' }}>
            AI Care Plan
          </h3>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {guidanceCards.map((card, i) => (
              <div key={i} className="p-4 rounded-xl border"
                style={{ borderColor: 'rgba(92,79,74,0.1)' }}
              >
                <div className="flex items-center gap-2 mb-2">
                  <card.icon size={18} style={{ color: '#5C766D' }} />
                  <span className="font-medium" style={{ color: '#5C4F4A' }}>
                    {card.title}
                  </span>
                </div>
                <p className="text-sm" style={{ color: '#5C4F4A80' }}>
