  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <InputField label="Age" name="AGE" icon={User} placeholder="e.g. 50" />
        <div className="relative group">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <ToggleBtn name="SMOKING" label="Smoking" />
        <ToggleBtn name="YELLOW_FINGERS" label="Yellow Fingers" />
        <ToggleBtn name="ANXIETY" label="Anxiety" />
        <ToggleBtn name="PEER_PRESSURE" label="Peer Pressure" />
        <ToggleBtn name="CHRONIC_DISEASE" label="Chronic Disease" />
        <ToggleBtn name="FATIGUE" label="Fatigue" />
        <ToggleBtn name="ALLERGY" label="Allergy" />
        <ToggleBtn name="WHEEZING" label="Wheezing" />
        <ToggleBtn name="ALCOHOL_CONSUMING" label="Alcohol Consuming" />
        <ToggleBtn name="COUGHING" label="Coughing" />
        <ToggleBtn name="SHORTNESS_OF_BREATH" label="Shortness of Breath" />
        <ToggleBtn name="SWALLOWING_DIFFICULTY" label="Swallowing Diff." />
        <ToggleBtn name="CHEST_PAIN" label="Chest Pain" />
      </div>
          ...
          {showAdvanced && (
            ...
            >
              <div className="p-4 mt-4 glass-panel rounded-xl border-purple-500/30 neon-glow-purple space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <InputField label="Radius" name="radius" icon={Info} placeholder="Mean Radius" step="0.01" />
                  <InputField label="Texture" name="texture" icon={Info} placeholder="Mean Texture" step="0.01" />
                  <InputField label="Area" name="area" icon={Info} placeholder="Mean Area" step="0.1" />
                  <InputField label="Smoothness" name="smoothness" icon={Info} placeholder="Mean Smoothness" step="0.001" />
                  <InputField label="Concavity" name="concavity" icon={Info} placeholder="Mean Concavity" step="0.001" />
                </div>
              </div>
            </motion.div>
          )}
